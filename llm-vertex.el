  ...
  (unless (and (llm-vertex-key provider)
    ...
    (let ((result (string-trim (shell-command-to-string (concat llm-vertex-gcloud-binary " auth print-access-token")))))
      (when (string-match-p "ERROR" result)
        (signal 'llm-provider-error (list (format "Could not refresh gcloud access token, received the following error: %s" result))))
          ...
          (:functionCallingConfig
           ,(append
             (list :mode (pcase (llm-tool-options-tool-choice options)
                           ...
                           ('any "ANY")
                           ((pred stringp) "ANY")
                           (_ (signal 'llm-not-supported (list (format "Unknown tool choice option: %s" (llm-tool-options-tool-choice options)))))))
             (when (stringp (llm-tool-options-tool-choice options))             `(:allowedFunctionNames [,(llm-tool-options-tool-choice options)]))))))))
