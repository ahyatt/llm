...
;;
;; To use this, create a prompt with `defprompt', and then fill it with
;; `llm-prompt-fill'.
;;
...
;;
;; When you want to call the LLM, you can use `llm-prompt-fill', which will
;; fill a fixed percentage of the context window `llm-prompt-default-max-pct' with
...
generator."
  (with-temp-buffer
    (let ((prompt-text (gethash name llm-prompt-prompts))
      ...
      (insert prompt-text)
      (let* ((vars (llm-prompt-variables-to-markers))
