...
function you should call ~llm-should-send-function-result-back~, and if the result
is non-~nil~, then you should call back into the LLM with ~llm-chat~ or equivalent
without modifying the prompt (which has already been set up for this in the
function call).  If ~llm-should-send-function-result-back~ is ~nil~, then the prompt
can be appended to as normal.  The results vary by provider; some, like Open AI,
require this to happen, others like Claude, will break if you try to send again
with modifying the prompt.
