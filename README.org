...
the context window we want to fill.
...
each will get an equal chance.  This algorithm is the contribution of David
Petrou.
...
So, to illustrate with this example, here's how the prompt will be filled:
  ...
  3. Run a lottery with all tickets and choose one of the remaining variables to
     fill.  Fill the variable with one entry retrieved from a supplied
     generator.
  4. Goto 2
  ...
  #+begin_src emacs-lisp
  (llm-prompt-fill-text "Hi, I'm {{name}} and I'm here to say {{message}}"
